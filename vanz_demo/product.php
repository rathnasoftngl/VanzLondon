...
<?php
$wval = $_REQUEST['w'];
if(isset($_REQUEST['w']) && isset($_REQUEST['cid']))
{ $scid = $_REQUEST['cid'];
	$sel_catname = mysql_fetch_row(mysql_query("select * from product_category where pid=$wval"));
	$mname = $sel_catname[1];
	$sel_subcatname = mysql_fetch_row(mysql_query("select * from sub_product_category where scid=$scid"));
	$subname = $sel_subcatname[2];
	$qry = "SELECT * from product_details where prod_type=$wval and prod_sub_cat=$scid";
...
if(is_numeric($wval))
{
	$sel_catname = mysql_fetch_row(mysql_query("select * from product_category where pid=$wval"));
	$mname = $sel_catname[1];
	$qry = "SELECT * from product_details where prod_type=$wval";
}

else
{   $mname = $wval;

 $qry = "SELECT p.* from product_details p inner join product_category pc on pc.pid=p.prod_type where pc.product_name='".$wval."'";
...
<a href="product.php?w=<?php echo $wval;?>" ><li><?php echo $mname;?> ></li></a>
<?php if(isset($_REQUEST['w']) && isset($_REQUEST['cid'])) { ?>
<a href="product.php?w=<?php echo $wval;?>&cid=<?php echo $scid;?>" ><li><?php echo $subname;?> ></li></a> <?php }?>
</ul>
</div>
</div>
...
<div class="container" style="margin-top:20px;">
	<div class="row">
		<div class="col-md-3">
			<!--<img class="img-responsive" src="images/left.jpg" alt="" style="width:100%;" >-->
		</div>
        <div class="col-md-9">
        <!--<img class="img-responsive" src="images/shoes_banner.jpg" alt="" style="width:100%;" >-->
            ...
            <?php $i=1;
			$sel_products = mysql_query($qry);
			if(mysql_num_rows($sel_products)>0)
			{
			while($exe_products = mysql_fetch_array($sel_products))
			{
			?>
            <div class="col-md-4">
            	<a href="single.php?pid=<?php echo $exe_products['id']; ?>"><div class="prod">
                <img class="img-responsive" src="images/prod1.jpg" alt="" style="width:100%;" >
                <p class="pname">Name : <?php echo $exe_products['prod_name']; ?></p>
                <p>£ <?php echo $exe_products['prod_price']; ?></p>
                </div></a>
            </div>
           ...
           <div class="col-md-8" align="center" style="margin-top:30px;">
           <span  style="padding-top:20px; font-size:16px; font-weight:600;">  No Items found! </span> 
           <p>&nbsp;</p>
           <p><a href="index.php" title="Vanz London"><input type="button" value="Continue Shopping &raquo;" class="btn_check" /></a></p></div>	 <?php } ?>
		</div>
	</div>
</div>            
...
<!-- Mirrored from p.w3layouts.com/demos/mihstore/web/ by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 01 Jul 2016 12:28:47 GMT -->
</html>
