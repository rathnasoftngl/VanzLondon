...
<script async type='text/javascript' src='../../../../cdn.fancybar.net/ac/fancybar6a2f.js?zoneid=1502&amp;serve=C6ADVKE&amp;placement=w3layouts' id='_fancybar_js'></script>
 ...
 <script>
 $(function() {
	/* var iniqty = $('#Qty_1').find('option:selected').val();
	 $('.qty_tot').text(iniqty); */
 $('#Qty_1').change(function() {
    
	iid = this.className;
	qty = $(this).val();
	formData = "tempid="+iid+"&qty="+qty;
	 //alert(formData);
	/*$.ajax({
     type: 'POST',
     url: 'ajaxAction.php',
...
{ 
    var sum = 0.0; //var qtytot;
    $('#cart_table > tbody  > tr#cart_td').each(function() {
        var qty = $(this).find('option:selected').val();
        var price = $(this).find('.price').val();
        var amount = (qty * price);
        sum += amount; 
		 //qtytot+=qty;
		//$('.amt_tot').text(''+parseFloat(amount)); 
        $(this).find('.amount').text(amount); 
		
    });
    //just update the total to sum  
   $('.tot_price').text(parseFloat(sum));
	//$('.qty_tot').text(qtytot); 
...
{ 
	/*var r=confirm("Remove Item from Bag?");
	if(r==true)
	{
...
<a href="#"><li>Shopping Bag ></li></a></ul>
</div>
</div>
...
<div class="container" style="margin-top:20px;">
	<div class="row">
		<div class="col-md-12">
        <p style="padding-bottom:20px;"> <span style="font-size:18px;">SHOPPING BAG</span>
       
        <br/> <br/><?php  if($itemsinbasket>0) { ?>   
        
        <?php  include("inc-cart.php");  } else { ?> Your bag contains 0 items
       <?php include("inc-cart1.php"); } ?> </p>
